  ...
  <div class="course-open">
    ...
    <el-form :inline="true" :model="searchForm" class="search-form">
      <el-form-item label="课程号">
        <el-input v-model="searchForm.id" placeholder="课程号" clearable />
      </el-form-item>
      <el-form-item label="课程名">
        <el-input v-model="searchForm.name" placeholder="课程名" clearable />
      </el-form-item>
      <el-form-item label="学分下限">
        <el-input-number v-model="searchForm.creditMin" :min="1" :max="10" />
      </el-form-item>
      <el-form-item label="学分上限">
        <el-input-number v-model="searchForm.creditMax" :min="1" :max="10" />
      </el-form-item>
      <el-form-item>
    ...
    <el-table :data="results" border stripe v-loading="loading">
      ...
      <el-table-column label="操作" width="120">
        <template #default="{ row }">
          <el-button size="small" type="success" @click="handleOpen(row)">开设</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const handleOpen = async (row) => {
  try {
    await axios.post('/api/teacher/openings', { courseId: row.id })
    ElMessage.success('开设成功')
  } catch (e) {
    ElMessage.error(e.response?.data?.message || '开设失败')
  }
}
...
const handleReset = () => {
  Object.assign(searchForm, { id: '', name: '', creditMin: undefined, creditMax: undefined, fuzzy: false })
  results.value = []
  total.value = 0
}
</script>
